...
void CViewRender::RenderView( const CViewSetup &view, int nClearFlags, int whatToDraw )
{
	...
	// VR TODO: is this where we hack in the 3d view and maybe the warp shaders
...
void CViewRender::DrawScope( const CViewSetup &viewSet )
{
	...
	{
		VectorCopy(localPlayer->Weapon_ShootPosition(), scopeView.origin);
		VectorCopy(VR_Controller()->weaponOrientation(), scopeView.angles);
	}
	...
	scopeView.x = 0;
	scopeView.y = 0;
	scopeView.fov = 45; //hard coded for the moment...
